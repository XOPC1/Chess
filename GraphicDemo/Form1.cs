        ...
        private void ChessBoardPaint(object sender, PaintEventArgs e)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                for (int i = 0; i <= 7; i++)
                {
                    ...
                    {
                        Rectangle rectCell = new Rectangle(30 + 60 * i, 30 + 120 * j, 60, 60);
                        g.FillRectangle(Brushes.Tomato, rectCell);
                    }
                    if (i % 2 == 0)
                    {
            ...
            for (int i = (int)'A'; i <= (int)'H'; i++)
            {
                ...
                g.DrawString(s, Hello_font, Brushes.DarkCyan, 45 + (i - (int)'A') * 60, 5);
            }
            for (int i = (int)'1'; i <= (int)'8'; i++)
            {
                string s = ((char)i).ToString();
                g.DrawString(s, Hello_font, Brushes.DarkCyan, 5, 45 + (i - (int)'1')*60);
            }
